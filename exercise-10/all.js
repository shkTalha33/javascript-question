...
const getTags = (accounts) => {
    return accounts[0].tags.flat()
}

// console.log(getTags(accounts))


const isTagExist = (accounts) => {
    return accounts[0].tags.flat().includes("consectetur")
}

// console.log(isTagExist(accounts))

const allTags = (accounts) => {
    return accounts.map((({tags} )=> {
        return tags.join(', ')
    })).join(', ')
}


// console.log(allTags(accounts))

const allFreinds = (accounts) => {
    return accounts.map((account) => {
        return account.friends.map(({name}) => name)
    }).flat()
}

// console.log(allFreinds(accounts))


const freindsListOnConditionCheck = (accounts) => {
    return accounts.filter((account) => {
        return parseInt(account.balance.replace(/\D/g,'')) > 1500 && account.age <= 30
    })
    .map(({friends}) => friends)
    .flat()
}

console.log(freindsListOnConditionCheck(accounts))