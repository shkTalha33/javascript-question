const users = [{
  ...
  const checkIsEmailValid = (users) => {
    return users.every(user => (/[^@]+@[^\.]+../).test(user.email))
  }

//   console.log(checkIsEmailValid(users))


const isIpAddressIsInRange = (users) => {
    return users.filter(({ip_address} )=> {
        return ip_address.split('.')[0] < 128
    }).length
}

// console.log(isIpAddressIsInRange(users))


const findPostionOfBTypeIPAdress = (users) => {
    return users.findIndex(({ip_address}) => {
    const firstBClass = ip_address.split('.')[0]
    return firstBClass >= 128 && firstBClass < 192 
    
    })
}

// console.log(findPostionOfBTypeIPAdress(users))


const findUserByEmail = (users) => {
    return users.find(({email}) => {
        return email.includes('soundcloud')
    })
}

// console.log(findUserByEmail(users))

const replaceAllItemsOfAray = (users) => {
    return users.fill("deleted")
}

console.log(replaceAllItemsOfAray(users))