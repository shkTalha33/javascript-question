...
const findUserId = (users) => {
    return users.find((user) => {
        return  user.firstName  === "Madison" && user.lastName === "Marshall" 
    })
}

// console.log(findUserId(users))


const firstComment = (users) => {
    return users.find((user) => {
        return user.id === comments[0].userId
    })
}

// console.log(firstComment(users))


const commentFind = (text) => {
    return users.find(user => user.id === comments.find(comment => comment.text === text).userId)
}

// console.log(commentFind("OK great thanks"))


const addUserFullnameInComment = (comments) => {
    return comments.map(comment => {
        const {firstName, lastName} = users.find(user => user.id === comment.userId)
        return {...comment, fullName : `${firstName} ${lastName}` }
    })
}

// console.log(addUserFullnameInComment(comments))


const userWhoDidnotComment = (users) => {
    return users.filter(user => {
        return !comments.find(comment => comment.userId === user.id)
    })
}

console.log(userWhoDidnotComment(users))